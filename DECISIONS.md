## [CHOIX] Support PWA étendu aux vues d'entrée hors partie — partial `@include` plutôt que duplication ou layout partagé

**Contexte :** suite de `feat/pwa-support` — `resources/views/partials/pwa-head.blade.php` (nouveau), `resources/views/{home,lobby/index,game/history,auth/login}.blade.php`, `resources/views/layouts/game.blade.php`.

**Symptôme / Problème :** trois points à vérifier avant toute modification, tous explicitement demandés plutôt que présupposés. (1) Vérifier la vue réellement servie par la route `/` : `routes/web.php:18` confirme `Route::get('/', fn () => view('home'))->name('home')` — l'ambiguïté "welcome.blade.php vs landing.blade.php" documentée plus bas dans ce fichier (entrée `fix/timers-broadcast-sync`) est un état historique déjà résolu : `git log --follow` sur `home.blade.php` montre un commit `cfcadbd` ("chore(views): rename welcome to home, remove orphan landing.blade.php") postérieur à cette décision — `welcome.blade.php` n'existe plus, `landing.blade.php` non plus, aucune confusion actuelle. (2) Identifier toutes les vues autonomes servant de point d'entrée hors partie : 4 trouvées — `home.blade.php` (`/`), `lobby/index.blade.php` (`/lobby`), `game/history.blade.php` (`/game/{code}/history`, oubliée par la Phase 59 précédente qui ne couvrait que le flux en partie), `auth/login.blade.php` (`/login`). (3) Vérifier si une vue de login dédiée existe : oui — `routes/web.php:19` (`Route::get('/login', fn () => view('auth.login'))`) sert une vraie vue Blade avec son propre document HTML, `/auth/google` (ligne 24) n'est que le controller qui initie la redirection OAuth Google, sans vue intermédiaire. Pas de vue à inventer.

**Cause / Alternatives :** le bloc PWA (manifest + 4 meta iOS + script d'enregistrement SW) devait être ajouté identique dans 4 nouveaux fichiers, en plus du bloc déjà présent dans `layouts/game.blade.php` (Phase 59) — 5 emplacements au total. Trois options : (a) copier-coller le bloc dans chacun des 4 nouveaux fichiers, en gardant l'original inline dans `layouts/game.blade.php` ; (b) créer un layout Blade partagé (`@extends`) que toutes ces vues adopteraient ; (c) factoriser le bloc dans un partial `@include`, réutilisé dans les 5 emplacements. (a) a été explicitement écartée par la demande elle-même ("ne duplique pas le CSS/HTML par copier-coller aveugle"), et créerait un risque de divergence future (un correctif iOS appliqué à un seul des 5 fichiers). (b) a été explicitement écartée par la demande ("sans créer de layout partagé — hors scope, trop risqué à ce stade") — un `@extends` commun imposerait la même structure `<html>`/`<body>`/scripts aux 4 vues autonomes, qui ont aujourd'hui des stacks radicalement différentes (`home.blade.php` charge Alpine/GSAP/ScrollTrigger via CDN sans bundle Vite complet, les 3 autres chargent `app.js` via `@vite`), un risque de régression bien réel que la demande a raison d'écarter à ce stade.

**Fix / Décision :** option (c) retenue. `resources/views/partials/pwa-head.blade.php` créé avec le contenu exact du bloc précédemment inline dans `layouts/game.blade.php` (manifest, meta iOS, theme-color, script d'enregistrement SW). Inclus via `@include('partials.pwa-head')` dans les 4 nouvelles vues ET dans `layouts/game.blade.php` lui-même (le bloc inline de la Phase 59 y a été remplacé par le même include — source unique désormais, plus de divergence possible). `@include` a été jugé sans le risque structurel de (b) : contrairement à `@extends`, un `@include` ne modifie ni la structure du document (`<html>`/`<head>`/`<body>`), ni le scope de layout — chaque vue garde l'intégralité de son `<head>`/`<body>`/scripts existants, seul un fragment statique supplémentaire est spliced au même endroit dans chacune. Aucune vue n'a eu sa structure, ses scripts existants ou son CSS touchés au-delà de l'ajout de cette seule ligne `@include`.

**Leçon :** une consigne qui écarte explicitement "layout partagé" pour limiter le risque ne doit pas être lue comme écartant toute factorisation — un `@include` Blade est une simple substitution de template à la compilation, sans les implications structurelles (`@extends`, sections, stacks partagés) qui motivaient l'exclusion du layout. Distinguer ces deux mécanismes Blade permet de choisir le niveau de factorisation adapté au risque réellement identifié, plutôt que de sur-interpréter une consigne de prudence en interdiction totale de factoriser.

**Statut :** 🔵 Choix assumé

---

## [CHOIX] Support PWA — périmètre limité à `layouts/game.blade.php` + enregistrement SW indépendant de WebPush

**Contexte :** `feat/pwa-support` — `public/manifest.json` (nouveau), `public/sw.js` (étendu), `resources/views/layouts/game.blade.php`.

**Symptôme / Problème :** deux ambiguïtés non tranchées par la demande initiale. (1) La demande pointait vers "`resources/views/layouts/*.blade.php` — identifie le bon" pour ajouter le manifest/meta iOS/enregistrement SW ; or `layouts/` ne contient qu'un seul fichier (`game.blade.php`), utilisé uniquement par les 9 vues du flux de partie (`waiting-room` → `finished`, confirmé par `grep @extends`). `home.blade.php` (landing), `lobby/index.blade.php` et `auth/login.blade.php` sont des documents HTML autonomes (leur propre `<html><head>`), hors de `resources/views/layouts/`. (2) Un service worker WebPush existait déjà (`public/sw.js`, handlers `push`/`notificationclick` uniquement, aucun `install`/`activate`/`fetch`) et son enregistrement (`resources/js/push-notifications.js::registerPush()`, appelé depuis `app.js`) est conditionné à la présence de la meta `vapid-public-key`, elle-même injectée uniquement `@isset($player)` dans `layouts/game.blade.php` — c'est-à-dire déjà scopé au même périmètre que (1), mais couplé à la demande de permission de notification.

**Cause / Alternatives :** Pour (1), étendre le manifest/SW à `home`/`lobby`/`auth` aurait nécessité soit de les migrer vers un layout partagé (hors périmètre — refactor non demandé, risque de régression visuelle sur des pages qui n'utilisent pas le bundle Vite standard), soit de dupliquer le `<link rel="manifest">` et les meta iOS dans 3 fichiers supplémentaires non couverts par la consigne explicite. Pour (2), deux options : (a) modifier `registerPush()` pour dissocier l'enregistrement du SW de la demande de permission push, ou (b) ajouter un second appel `navigator.serviceWorker.register('/sw.js')` indépendant dans le layout, en s'appuyant sur l'idempotence de l'API (même script + même scope → même `ServiceWorkerRegistration`, pas de double instance, pas d'erreur).

**Fix / Décision :** (1) Périmètre limité aux vues étendant `layouts/game.blade.php` — documenté dans TODO.md Phase 59 et ajouté à la ROADMAP (BUGS_AND_ROADMAP.md) pour extension future si le besoin d'installabilité sur la landing/lobby est confirmé. (2) Option (b) retenue : ajout d'un `<script>` autonome et inconditionnel (`if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js')`) dans `layouts/game.blade.php`, sans toucher à `push-notifications.js` ni à `registerPush()` — l'installabilité PWA ne dépend ainsi plus de l'acceptation de la permission de notification par l'utilisateur. `public/sw.js` a été étendu (jamais remplacé) : `install`/`activate`/`fetch` ajoutés à côté des handlers `push`/`notificationclick` existants, cache-first sur `/build/*`+`/icons/*`+polices, network-first sans cache sur le reste, exclusion explicite de `/broadcasting/auth`, `/game/*/state` et `/api/*`.

**Leçon :** quand une consigne pointe vers un dossier contenant un seul fichier candidat ("identifie le bon"), vérifier quand même par `grep @extends` que ce fichier couvre bien tout le flux utilisateur attendu avant de considérer le périmètre complet — ici 3 pages d'entrée (landing, lobby, login) restent hors PWA, ce qui est probablement acceptable pour un premier jalon (l'installabilité importe surtout une fois en partie) mais mérite d'être explicite plutôt que découvert plus tard. Par ailleurs, `navigator.serviceWorker.register()` étant idempotent par (script URL, scope), un second point d'enregistrement n'est jamais un bug en soi — mais vaut toujours la peine d'être documenté explicitement plutôt que laissé comme une coïncidence silencieuse, surtout quand un enregistrement existant est conditionné à un flux fonctionnel sans rapport (ici : permission de notifications push).

**Statut :** 🔵 Choix assumé

---

## [RÉSOLU] Victoire Loups déclarée avant résolution du tir du Chasseur (partie MGUIVJ)

**Contexte :** `fix/win-condition-hunter-timing` — `app/Services/WinConditionChecker.php`, `app/Jobs/ProcessHunterTurn.php`, `app/Services/PhaseManager.php`, `tests/Unit/Services/WinConditionCheckerTest.php`, `tests/Feature/Game/HunterTest.php`. Bug remonté depuis la partie MGUIVJ (26/07/2026) : 5 vivants (2 Loups, Chasseur, 2 non-loups) — les Loups tuent le Chasseur en Nuit 3, ce qui fait tomber l'effectif à exactement 2 Loups vs 2 non-loups (parité). La partie s'est terminée immédiatement en "Victoire des Loups" sans aucune trace du tir du Chasseur dans l'historique, alors qu'un `GameAction hunter_pending` avait bien été créé en parallèle.

**Symptôme / Problème :** `WinConditionChecker::check()` n'avait aucune notion de tir de Chasseur en attente — il tranche la victoire Loups/Village dès que la parité (`aliveWerewolvesCount() >= aliveVillagersCount()`) est atteinte, en ne lisant que l'état `is_alive` courant des joueurs. Dans `ProcessNightActions::handle()`, la mort du Chasseur (`eliminate()`) et la création du `hunter_pending` GameAction ont lieu *avant* l'appel à `check()` — dès que cette mort atteint la parité Loups, `check()` termine la partie et fait un `return` immédiat, avant que `ProcessWitchTurn`/`ProcessNightEnd`/`ProcessHunterTurn` ne soient jamais dispatchés. Le Chasseur ne reçoit donc jamais son tour (`HunterTurnStarted`), et le `hunter_pending` reste orphelin en base pour toujours (la partie étant `finished`, plus rien ne le consomme). Reproduit à l'identique côté vote de jour (`VoteService::dispatchDayVoteConsequences()`), structure de code identique. Confirmé par reproduction locale (2 tests, nuit et jour) avant tout fix — logs à l'appui : `hunter_pending still in DB: true` / `hunter_shot ever recorded: false` / `ProcessNightEnd dispatched: false`.

Audit des 7 call sites de `WinConditionChecker::check()` dans `app/` :

| Call site | `hunter_pending` déjà présent/pertinent avant l'appel ? | Verdict avant fix |
|---|---|---|
| `ProcessNightActions::handle()` | Oui — créé juste avant l'appel | 🔴 Affecté (confirmé par reproduction) |
| `VoteService::dispatchDayVoteConsequences()` (branche 0 vote) | Oui | 🔴 Affecté (même structure) |
| `VoteService::dispatchDayVoteConsequences()` (branche élimination) | Oui | 🔴 Affecté (confirmé par reproduction) |
| `WitchAction::act()` | Oui — créé par `finalizeOrdinaryVictim()`/branches différées avant l'appel | 🔴 Affecté (même structure) |
| `WitchAction::finalizeTimedOutVictim()` | Oui | 🔴 Affecté (même structure) |
| `ProcessHunterTurn::handle()` (tout en haut, avant `HunterTurnStarted`) | Non — l'appelant (`ProcessNightEnd`/`dispatchDayVoteConsequences`) supprime déjà le `hunter_pending` avant de dispatcher ce Job (guard anti double-dispatch) | 🟠 Angle mort apparenté — même défaut, une étape plus loin dans la chaîne |
| `ProcessHunterAutoAction::handle()` | Non — le tour du Chasseur est déjà définitivement clos (tiré ou timeout) à ce stade | 🟢 Sain — vérification terminale légitime, inchangée |
| `PhaseManager::endNight()` | Non — atteint uniquement quand `ProcessNightEnd` n'a trouvé aucun `hunter_pending` | 🟢 Sain — garanti par construction, inchangé |

**Cause / Alternatives :** Aucune alternative structurelle envisagée pour le mécanisme central — `check()` doit connaître l'existence d'un tir en attente pour pouvoir la reporter, il n'y avait pas de raccourci. Deux options de correction ont été comparées (arbitrage demandé explicitement avant implémentation) : (1) faire remonter la logique de dispatch de la chaîne Chasseur devant chaque `check()` concerné, ou (2) faire porter le report par `check()` lui-même via une requête sur `hunter_pending`, complétée d'un second `check()` après résolution du tir. L'option 2 a été retenue (déjà le pattern en place ailleurs pour un autre win-condition timing gap, cf. entrée "Victoire des Amoureux non déclenchée... SFICZ8" plus bas dans ce fichier) car elle ne touche à aucune mécanique de double-dispatch existante (le `hunter_pending` est supprimé avant dispatch de `ProcessHunterTurn`, indispensable pour éviter un double tour).

Piste explorée et abandonnée pendant l'implémentation : ajouter un `check()` inconditionnel en tête de `PhaseManager::startNight()`, par symétrie stricte avec `endNight()` (qui a ce garde depuis le fix SFICZ8). Cassait 5 tests existants (`CupidonNightIntegrationTest`, `MayorSuccessionTest`, `NightResyncTest`) qui démarrent légitimement une nuit avec un effectif sans loup pour tester la seule mécanique de transition — `check()` y déclarait alors à tort une victoire Village. `startNight()` n'a en réalité pas besoin de ce garde : `ProcessHunterAutoAction::handle()` appelle déjà `check()` inconditionnellement avant toute branche jour/nuit, pour tout dénouement de tir (volontaire ou timeout) — c'est ce Job partagé nuit/jour qui joue le rôle du filet, pas une paire symétrique `endNight()`/`startNight()`. Seule la branche de repli de `ProcessHunterTurn::handle()` (Chasseur invalide/déjà résolu, qui appelle `startNight()`/`endNight()` directement sans repasser par `ProcessHunterAutoAction`) avait besoin d'un `check()` local dédié.

**Fix / Décision :**
1. `WinConditionChecker::check(Game $game, ?int $awaitingHunterId = null)` : la victoire Loups/Village est reportée (retour `null`/`false`, aucune persistance) tant qu'un `GameAction hunter_pending` existe pour `$game->round`, ou que `$awaitingHunterId` est fourni. La victoire Amoureux (calculée en premier, avant ce guard) reste inconditionnelle et prioritaire — jamais reportée, conforme à SPEC_CUPIDON.md §6.
2. `ProcessHunterTurn::handle()` : l'appel principal passe désormais `awaitingHunterId: $this->hunterId` — nécessaire car à ce stade l'appelant a déjà supprimé le `hunter_pending` (guard anti double-dispatch), donc `check()` ne verrait plus aucun tir en attente sans ce paramètre explicite. La branche de repli (Chasseur invalide/déjà résolu) fait un second `check($game)` sans ce paramètre, pour de vrai cette fois, avant de transitionner.
3. `PhaseManager::startNight()` : aucun changement de comportement — un commentaire documente pourquoi un `check()` inconditionnel y serait une erreur (voir ci-dessus).
4. Tests : 5 ajoutés à `WinConditionCheckerTest` (parité différée avec `hunter_pending` en base ; différée via `awaitingHunterId` sans ligne en base ; tir sur un loup annule la victoire en attente ; absence de changement de parité confirme la victoire après résolution ; victoire Amoureux toujours immédiate malgré un `hunter_pending`). 1 test bout-en-bout ajouté à `HunterTest` reproduisant exactement le scénario MGUIVJ (2 Loups + Chasseur + 2 villageois, mort du Chasseur atteint la parité, le Chasseur reçoit bien son tour, tire sur un Loup, la partie continue). 319/319 tests verts (Reverb démarré localement pour la suite complète — mêmes échecs `WinConditionCheckerTest` habituels si Reverb absent, indépendants de cette tâche, voir DECISIONS.md "Cupidon Étape 7").

**Leçon :** un mécanisme d'"action en attente" (`hunter_pending`) créé pour différer une conséquence (ici initialement : succession du Maire, déjà documenté dans "Chasseur Maire — tir avant succession du maire") doit être audité pour *toutes* les décisions qui peuvent court-circuiter le round en cours, pas seulement celle qui a motivé sa création initiale — `WinConditionChecker::check()` est une mutation d'état terminal au même titre qu'une succession de Maire, et ignorait complètement ce garde. Comme pour les bugs Chasseur/Maire précédents, un même défaut de garde peut exister à plusieurs call sites structurellement identiques (ici 5 sur 7) : auditer tous les appelants d'une méthode avant de considérer un fix ponctuel suffisant. Enfin, une "symétrie" apparente entre deux méthodes miroir (`endNight()`/`startNight()`) n'implique pas que le même garde défensif y soit également sûr à ajouter — leurs appelants réels (et les rosters de test qu'ils utilisent) peuvent diverger suffisamment pour qu'un garde légitime d'un côté casse des usages légitimes de l'autre ; vérifier par l'exécution de la suite complète avant de généraliser un pattern défensif.

**Statut :** ✅ Résolu

---

## [CHOIX] Musiques d'ambiance — crossfade simulé cross-page + mute via `el.muted` (pas `volume`) pour l'autoplay

**Contexte :** `feat/ambient-music` — `resources/js/audio-manager.js` (nouveau), `resources/js/game-state.js`, `resources/js/app.js`, `vite.config.js`, `resources/views/layouts/game.blade.php`, `resources/views/game/{waiting-room,role-reveal,mayor-election,night,day,finished}.blade.php`, `resources/views/home.blade.php`, `resources/views/lobby/index.blade.php`, `resources/views/components/toast.blade.php`, `public/audio/*.ogg`.

**Symptôme / Problème :** deux contraintes de la spec semblaient en tension avec l'architecture existante. (1) "Crossfade ~1-1.5s entre deux `<audio>` superposés" suppose un contexte audio qui survit à la transition — or **toutes** les transitions de phase de ce projet sont des navigations serveur complètes (`window.location.href`, jamais de SPA), qui détruisent le document et donc tout `<audio>` en cours. (2) "Ne jamais bloquer le premier son sur `window.onload`" implique un rejet de `play()` sans geste utilisateur — mais si l'utilisateur est **muet** (préférence `localStorage` d'une session précédente), on voudrait quand même que l'élément audio soit synchronisé dès le chargement de la page (pour qu'un unmute ultérieur soit instantané), sans dépendre d'un geste.

**Cause / Alternatives :** (1) Chaque vue Blade correspond déjà 1:1 à une "phase parente" au sens de la table de mapping (le routing serveur `redirectToCurrentPhase()`/statuts intermédiaires `wolves_turn`/`processing_night`/`processing_day` ne créent jamais de nouvelle page — seuls `NightStarted`/`DayStarted`/`MayorElectionStarted`/`GameFinished` déclenchent une navigation). Il n'existe donc **aucune transition de piste à l'intérieur d'une même page** dans ce projet : pas besoin d'un helper JS `status → phase` (aucun n'existait, `ChatService::mapStatusToPhase()` est côté PHP et pour un usage différent) — chaque vue appelle simplement `crossfadeTo('sa_piste')` une fois à son propre chargement. (2) Pour l'autoplay, le volume (`el.volume`) n'a **aucun effet** sur l'autorisation du navigateur — seul le booléen natif `el.muted = true` est reconnu par les moteurs (Chrome/Safari/Firefox) comme dispensant du geste utilisateur.

**Fix / Décision :**
1. **Crossfade simulé, pas persistant** : `_navigateTo()` (game-state.js) + les redirections locales (`role-reveal.blade.php`, `mayor-election.blade.php`) appellent `AudioManager.fadeOutCurrent()` juste avant `window.location.href`. La page suivante fait son propre `crossfadeTo('nouvelle_piste')` au chargement (fade-in depuis silence). Le résultat perçu par le joueur est un fondu enchaîné correct (le fade-out se termine pendant que la nouvelle page charge), mais ce n'est **pas** un unique contexte audio qui glisse en douceur — c'est un fade-out + un fade-in indépendants, bornés chacun par le cycle de vie de leur propre document. Aucune tentative de préserver `currentTime`/lecture entre les deux pages (impossible sans bfcache garanti).
2. **`el.muted` pilote l'autorisation navigateur, `el.volume` pilote uniquement la rampe de fondu** : `crossfadeTo()` fait toujours `incoming.muted = <préférence utilisateur>` puis `incoming.play()`, indépendamment du volume ciblé. Un utilisateur muet voit donc son `<audio>` démarrer silencieusement dès le chargement de la page (autoplay autorisé par le navigateur car `muted=true`), sans jamais passer par le mécanisme de retry — et un `unmute` ultérieur (`el.muted = false` déclenché par un vrai clic sur le toggle) est instantané, pas de nouvel appel `play()` nécessaire. Seul le cas non-muet + aucun geste préalable déclenche le rejet de `play()` et le retry sur prochain clic/tap/touche clavier (jamais `mousemove`, non reconnu comme geste d'activation).
3. **Portée du mapping strictement limitée aux 5 cas explicites de la spec** : `summary.blade.php`, `cancelled.blade.php`, `spectator.blade.php`, `history.blade.php` (tous héritent quand même du bouton toggle mute via `layouts/game.blade.php`) restent volontairement sans piste déclenchée — `winner.ogg` ne joue que sur `finished.blade.php` (issue avec vainqueur), jamais sur `cancelled.blade.php` (partie annulée, `winner_team = null`), la spec ne mentionnant que "peu importe l'issue (village/loups/amoureux)" sans jamais évoquer l'annulation.
4. **Bouton toggle mute** repositionné bas-droite (`bottom-20 md:bottom-4 right-4`, layouts/game.blade.php) après un premier essai en haut-gauche qui chevauchait des titres de page — décalé du bouton Quitter (haut-droite) et du footer nav mobile ; le composant `<x-toast>` a dû être repoussé (`bottom-4` → `bottom-36 md:bottom-20`) pour ne pas se superposer au nouveau bouton, les deux étant ancrés au même coin.

**Leçon :** avant d'implémenter un pattern de transition audio/vidéo "crossfade" dans une app à navigation serveur complète (pas de SPA), vérifier si l'architecture garantit réellement un contexte JS persistant entre les deux états — sinon le crossfade ne peut être qu'une approximation en deux temps (fade-out sortant + fade-in entrant sur des documents différents), ce qui est suffisant perceptuellement mais ne doit pas être présenté comme un vrai crossfade inter-documents. Par ailleurs, la confusion volume/mute pour l'autoplay est un piège classique : la politique d'autoplay des navigateurs teste le flag `muted`, jamais le niveau de volume — un `<audio>` à `volume: 0` non muet reste soumis aux mêmes restrictions qu'à `volume: 1`.
...
**Symptôme / Problème :** Contrairement à la victime directe (loups, poison Sorcière, vote village, tir du Chasseur), où c'est toujours l'appelant de `eliminate()` qui broadcaste `PlayerEliminated` après coup, la cascade de mort par chagrin se produit entièrement à l'intérieur de `PlayerEliminationService::eliminate()`, en dehors de tout appelant. Aucun des 4 call sites existants ne voit jamais cette mort cascadée — ils ne broadcastent que pour leur propre victime directe. Résultat concret : la liste des joueurs vivants affichée en live chez tout le monde ne se mettait même pas à jour quand l'amoureux mourait de chagrin (en plus de l'absence totale de toast), un symptôme uniquement visible en jouant une vraie partie avec Cupidon jusqu'à ce cas précis — jamais couvert par les tests unitaires existants de `PlayerEliminationServiceTest`, qui n'assertaient que sur `is_alive` et `hunter_pending`, jamais sur les events.

**Cause / Alternatives :** Même angle mort architectural que le bug Chasseur précédent — la cascade est un 5ᵉ point d'élimination qui n'est couvert par aucune des protections habituellement dupliquées aux 4 call sites (ici : le broadcast lui-même, pas seulement `hunter_pending`). Seule option cohérente avec `PlayerEliminationService` comme point d'entrée unique : faire broadcaster la cascade elle-même, exactement comme elle crée déjà elle-même le `hunter_pending` de l'amoureux cascadé.

**Fix / Décision :** Dans la branche cascade de `eliminate()`, après l'appel récursif `$this->eliminate($lover)` : `$lover->load('user')` (même geste que les 4 call sites existants, cohérence de pattern — `google_name` n'est en réalité plus dans `PlayerEliminated::broadcastWith()` depuis le fix vie privée du 2026-06-24, donc ce `load('user')` ne sert plus qu'à la notification, mais je l'ai gardé pour ne pas casser la symétrie avec le reste du code) puis `broadcast(new PlayerEliminated($game, $lover, 'heartbreak'))`, avant le bloc `hunter_pending` existant — jamais sur le `$player` du paramètre initial, qui reste sous la responsabilité de l'appelant (aucune duplication du broadcast normal). Côté client, `game-state.js::handlePlayerEliminated()` : branche `reason === 'heartbreak'` sur un toast public dédié ("💔 {pseudo} meurt de chagrin après la mort de son amoureux — c'était le {rôle}."), distinct du toast générique "💀 {pseudo} était le {rôle}". Aucun toast personnel séparé ajouté : contrairement au poison de la Sorcière (`DayStarted`, sessionStorage, nécessaire car la page `/night` est détruite par une redirection GSAP avant que le toast ne puisse s'afficher), la mort par chagrin n'a pas ce problème de timing — `player.eliminated` est un broadcast public immédiat, l'amoureux cascadé le reçoit comme tout le monde et voit déjà le toast public, plus la bannière de mort générique déjà gérée par l'event `i-was-eliminated` (inchangé, fonctionne pour n'importe quel `reason`).

**Leçon :** Un point d'entrée centralisé (`PlayerEliminationService`) qui gagne une branche de logique interne (ici la cascade) doit être audité pour *toutes* les responsabilités que les call sites externes assument normalement après une élimination — pas seulement celle qui a déjà causé un bug visible une première fois (`hunter_pending`). Le broadcast `PlayerEliminated` était le même angle mort que `hunter_pending`, simplement découvert dans un second temps parce qu'il ne casse aucune fonctionnalité de jeu bloquante (contrairement à un tir de Chasseur manqué) — juste un défaut de mise à jour live silencieux, plus difficile à remarquer en test.

**Statut :** ✅ Résolu

---

## [RÉSOLU] Chasseur mort de chagrin (cascade amoureux) ne tirait jamais

**Contexte :** `fix/hunter-heartbreak-death-shot` — `app/Services/PlayerEliminationService.php`, `tests/Unit/Services/PlayerEliminationServiceTest.php`, `tests/Feature/Game/CupidonTest.php`. Signalé : quand un joueur meurt et que son amoureux (lien Cupidon) encore vivant meurt "de chagrin" par cascade, si cet amoureux est le Chasseur, il ne tire jamais avant de mourir — contrairement à tous les autres chemins de mort du Chasseur (loups, poison Sorcière, vote village), qui lui donnent tous un dernier tir.

**Symptôme / Problème :** `PlayerEliminationService::eliminate()` pose `is_alive = false` sur le joueur passé en paramètre, puis récurse sur son amoureux (`lover_player_id`) si celui-ci est encore vivant — sans jamais vérifier si cet amoureux cascadé est le Chasseur. Les 4 call sites existants (`ProcessNightActions`, `WitchAction`, `VoteService::resolveDayVoteWinner`, cible directe du Chasseur lui-même) vérifient chacun `isHunter()` sur **leur propre victime directe** après avoir appelé `eliminate()`, et créent le `GameAction hunter_pending` correspondant — mais aucun d'eux ne voit jamais l'amoureux cascadé, qui meurt entièrement à l'intérieur du service, hors de leur portée. Angle mort du refactoring de centralisation de l'élimination (SPEC_CUPIDON.md §5) : la cascade a été ajoutée comme un pur effet secondaire de `is_alive`, sans reproduire la logique `hunter_pending` déjà dupliquée aux 4 endroits.

**Cause / Alternatives :** Pas d'alternative envisagée — dupliquer à nouveau la vérification `isHunter()` dans les 4 call sites pour couvrir le cas cascade aurait reproduit exactement le problème que `PlayerEliminationService` a été créé pour éliminer (un 5ᵉ endroit à maintenir, avec le même risque de bug silencieux si un jour un 6ᵉ call site apparaît). Le fix le plus cohérent avec l'architecture existante est de placer la vérification au seul endroit qui voit réellement la mort de l'amoureux cascadé : dans la branche cascade elle-même.

**Fix / Décision :** Dans `PlayerEliminationService::eliminate()`, uniquement dans la branche cascade (jamais sur le `$player` du paramètre initial, qui reste sous la responsabilité de l'appelant), après l'appel récursif `$this->eliminate($lover)` : si `$lover->isHunter()`, création du `GameAction hunter_pending` avec `round`/`phase` déterminés via `$lover->game->isNightPhase()` (jamais de lecture directe de `$game->status`). Le service ne dispatche rien — `ProcessNightEnd` (nuit) et `VoteService::dispatchDayVoteConsequences()` (jour) consomment déjà le `hunter_pending` par `game_id + round`, indépendamment de qui l'a créé, donc aucun changement nécessaire côté consommateurs. `RoleDistributor` limite le Chasseur à 0 ou 1 par partie : deux `hunter_pending` simultanés (victime directe + amoureux cascadé tous deux Chasseurs) sont structurellement impossibles.

Détail d'investigation notable pendant l'écriture des tests : côté nuit, `ProcessNightActions` ne consomme jamais lui-même le `hunter_pending` qu'il crée (ou que la cascade crée) — c'est `ProcessNightEnd`, dispatché plus tard, qui le lit puis le supprime. Côté jour, en revanche, `VoteService::resolveDayVote()` appelle `dispatchDayVoteConsequences()` **dans le même appel synchrone**, qui trouve le `hunter_pending` et le **supprime immédiatement** en dispatchant `ProcessHunterTurn` — sans attendre un job séparé. Un test qui asserte la présence du `GameAction hunter_pending` en base juste après `resolveDayVote()` échoue donc à tort (la ligne existe bien, brièvement, mais elle est déjà consommée avant que `resolveDayVote()` ne retourne) : la bonne assertion côté jour est le `Queue::assertPushed(ProcessHunterTurn::class, ...)` ciblant le bon `hunterId`, pas une lecture de `game_actions`.

**Leçon :** Quand une opération est centralisée dans un point d'entrée unique précisément pour éviter la duplication d'une vérification à plusieurs call sites (`PlayerEliminationService`, cf. SPEC_CUPIDON.md §5), toute nouvelle branche de logique ajoutée *à l'intérieur* de ce point d'entrée (ici la cascade amoureux) doit être traitée comme un 5ᵉ "call site" à part entière pour les vérifications qui accompagnent normalement chaque élimination (`hunter_pending`, et potentiellement d'autres à l'avenir) — elle n'est pas couverte gratuitement par le fait que les 4 call sites existants font déjà cette vérification sur leur propre victime. Plus généralement : avant d'écrire un test qui vérifie la persistance d'un `GameAction` "pending" (pattern `hunter_pending`), vérifier si le chemin testé consomme (delete) ce `GameAction` de façon synchrone avant de rendre la main, ou seulement via un Job différé — les deux patterns coexistent dans la codebase (nuit vs jour) et n'ont pas la même fenêtre d'observation en base.
...
## [RÉSOLU] Écran Voyante (seer_result) se referme sur un mauvais repère temporel

**Contexte :** `fix/seer-result-screen-resync` — `resources/views/game/night.blade.php`, `app/Jobs/ProcessSeerTurn.php`. Fait suite à un audit dédié (délai perçu de ~10-14s entre la fin du tour Voyante et le début du tour des Loups, testé en partie réelle à 6 joueurs avec `queue:work`/`reverb:start` actifs).

**Symptôme / Problème :** l'écran `seer_result` de la Voyante repassait automatiquement à `village_sleeping` via un `setTimeout(5000)` déclenché à la réception de `SeerResult`. Or `SeerResult` peut arriver dès mi-timer (`ProcessSeerAutoAction`, inspection de consolation), alors que le vrai passage aux Loups (`ProcessWerewolvesTurn`) n'a lieu qu'à `seerTimer + 2s` côté serveur. Écart mesuré en conditions réelles : `(seerTimer + 2) − (seerTimer/2 + 5)` ≈ 12-14s de "silence" côté client, perçu comme un délai anormal alors que le serveur se comporte exactement comme documenté (`SPEC_TIMERS.md §3.3`).

**Cause / Alternatives :** deux options envisagées, tranchées par audit préalable de `NightResyncService` (demandé explicitement avant tout choix d'implémentation) :
- **Option B (`NightResyncService`/`night_sub_phase`) écartée** — ce service n'est consommé que par `GameController::state()`, appelé uniquement au chargement de page et à la reconnexion Echo (`_loadState()` dans `game-state.js`, jamais en polling périodique). Ce n'est pas un flux temps réel pour un client déjà connecté ; le réutiliser aurait exigé d'introduire un polling `setInterval` inédit dans l'architecture — contraire au principe établi (transitions pilotées par événement serveur reçu, jamais par estimation/poll côté client).
- **Réutiliser directement l'event `WerewolvesTurnStarted` côté Voyante, tel que suggéré initialement dans le prompt du fix, s'est révélé impossible** : ce broadcast n'a lieu que sur le canal privé `game.{id}.werewolves` (`routes/channels.php` + `game-state.js` ligne ~239 : l'abonnement Echo à ce canal est conditionné à `isWolfEffective`). La Voyante ne s'y abonne jamais et ne peut structurellement pas recevoir cet event — le prompt supposait à tort qu'il était déjà accessible "ailleurs" pour tous les rôles.
- Étant donné qu'aucun signal serveur légitime n'existe (et ne doit exister, `SPEC_TRANSITIONS.md §9` : "seer_turn et werewolves_turn jamais publics") pour indiquer à la Voyante que le tour des Loups a commencé, la seule transition acceptable était déjà présente dans la vue : le bouton "J'ai compris" (`@click="nightPhase = 'village_sleeping'"`), dismiss 100 % volontaire côté joueur.

**Fix / Décision :** suppression pure et simple du `setTimeout(5000)` dans `night.blade.php` (`nightScreen()`, listener `seer-result`). Aucun remplacement par un nouvel event ou une nouvelle minuterie — la Voyante reste sur `seer_result` jusqu'à dismiss volontaire (bouton existant) ou jusqu'à la vraie fin de nuit (`DayStarted`, déjà géré globalement par `game-state.js` pour tous les rôles, redirection vers `/day`). Commentaire stale de `ProcessSeerTurn.php` ("timer complet + 5s") corrigé pour refléter la valeur réelle du code (`seerTimer + 2`) — coïncidence probable avec le `5000` du client, deux constantes "5s" non liées à l'origine du malentendu de l'audit initial.

**Leçon :** avant de proposer "attendre l'event X déjà écouté ailleurs dans le fichier" comme fix, vérifier sur QUEL canal (public vs privé par rôle) cet event est réellement diffusé et QUI s'y abonne côté client — un event écouté "ailleurs dans le même fichier" peut être structurellement inaccessible à un autre rôle si son canal Echo est conditionné (`if (isWolfEffective)`). Quand aucun signal serveur légitime n'existe pour une transition (et ne doit pas exister, pour préserver l'anti-fuite de rôle), le contrôle volontaire du joueur (bouton dismiss) est une solution valide — pas la peine d'inventer un mécanisme de resynchro pour un client qui n'a besoin que d'attendre le prochain vrai event global.
...
## [CHOIX] Statistiques admin — calcul des victoires par camp (feat/admin-stats)
**Contexte :** ajout des colonnes Victoires/Rôle favori (`admin/users/index.blade.php`), du donut victoires/défaites (`admin/users/show.blade.php`) et du bar chart de répartition des rôles (`admin/dashboard.blade.php`). `AdminUserController::winCaseSql()` détermine si une ligne `game_players` est une victoire.
**Symptôme / Problème :** aucune colonne `won`/`camp` n'existe sur `game_players` — le camp gagnant (`games.winner_team`) doit être recoupé avec le rôle du joueur au moment du calcul, avec plusieurs ambiguïtés non tranchées par la spec : (1) cas `lovers`, un camp transversal aux rôles classiques ; (2) faut-il compter une victoire pour un joueur mort avant la fin de la partie mais dont le camp a gagné ; (3) les parties `winner_team = null` (annulées) doivent-elles compter dans le dénominateur d'un taux de victoire ; (4) égalité de fréquence pour le "rôle favori" (mode statistique).
**Cause / Alternatives :**
1. `lovers` — alternative écartée : classer par rôle du joueur (loup/village) même en cas de victoire amoureux, ce qui aurait crédité une victoire aux DEUX camps classiques à la fois pour la même partie (incohérent). Retenu : un joueur gagne en `lovers` s'il a un `lover_player_id` non nul dans cette partie (couple formé), indépendamment de son rôle — cohérent avec `WinConditionChecker` où seuls les 2 amoureux vivants déclenchent cette victoire.
2. Joueur mort avant la fin — alternative écartée : n'exiger `is_alive = true` en fin de partie pour créditer la victoire, ce qui aurait exclu par exemple un Villageois tué en Nuit 1 dans une partie où le Village gagne quand même. Retenu : la victoire est créditée à tout joueur dont le camp (rôle ou lien amoureux) correspond à `winner_team`, vivant ou mort en fin de partie — c'est la convention usuelle du jeu de société (victoire de camp, pas de survie individuelle), et aucun événement/écran du jeu (`GameFinished`, `finished.blade.php`) ne fait cette distinction non plus.
3. Parties annulées — `winner_team = null` : exclues nativement du calcul (le `CASE` ne matche aucune branche, `ELSE 0`), et `show()` filtre en amont `whereNotNull('games.winner_team')` pour le dénominateur (victoires/défaites/taux) — une partie annulée n'est ni une victoire ni une défaite, elle ne doit pas non plus diluer le taux. La colonne "Parties jouées" de `index()` (déjà existante, `withCount('gamePlayers')`) reste inchangée et continue d'inclure les annulées : elle mesure la participation, pas la performance.
4. Rôle favori — égalité de fréquence tranchée par ordre alphabétique du nom de rôle (`ORDER BY COUNT(*) DESC, role ASC` dans la fenêtre `ROW_NUMBER()`) : choix arbitraire mais déterministe, documenté ici plutôt que laissé silencieux. Alternative écartée : ne rien afficher en cas d'égalité (jugé moins utile qu'un choix stable pour un tableau de bord).
**Fix / Décision :** `AdminUserController::winCaseSql()` (méthode privée partagée par `index()` et `show()`) encode ces 4 règles dans un unique `CASE` SQL réutilisé par agrégation (`SUM()`), jamais de `foreach` PHP sur des collections chargées en mémoire. Périmètre de fichiers strictement respecté (`app/Http/Controllers/Admin/`, `resources/views/admin/`, + `admin/layout.blade.php` pour le CDN Chart.js) : la logique reste dans les Controllers admin plutôt que dans un Service dédié (`app/Services/Admin/...`) qui aurait été hors du périmètre autorisé par la consigne — jugé non nécessaire vu la taille de la logique (une méthode privée de ~10 lignes, réutilisée 2 fois dans la même classe).
**Leçon :** dans un jeu à camps (pas de survie individuelle), une "victoire" côté stats admin doit être définie au niveau du camp en fin de partie, pas de l'état de vie du joueur — sinon les chiffres divergent silencieusement des écrans de fin de partie déjà vus par les joueurs. Pour un camp transversal aux rôles (`lovers`), préférer une colonne de lien explicite (`lover_player_id`) à une inférence par rôle.
**Statut :** 🔵 Choix assumé

## [CHOIX] Leaderboard admin — réutilisation du calcul de victoire (feat/admin-leaderboard)
**Contexte :** ajout de `admin/leaderboard` (`AdminLeaderboardController`, `app/Services/Admin/LeaderboardService.php`, `admin/leaderboard.blade.php`). `feat/admin-stats` était déjà mergée sur `dev` au moment de démarrer cette tâche (vérifié via `git log --merged`) — la consigne demandait donc de réutiliser la logique de victoire existante plutôt que de la redévelopper.
**Symptôme / Problème :** `AdminUserController::winCaseSql()` (le `CASE` SQL de calcul de victoire par camp, voir décision ci-dessus) était `private` — inutilisable tel quel depuis un Service distinct sans dupliquer la chaîne SQL.
**Cause / Alternatives :** trois options envisagées — (1) dupliquer le `CASE` SQL dans `LeaderboardService` (rejeté : viole explicitement la consigne "réutilise sa logique de calcul" alors que la branche est mergée, et deux copies auraient divergé au premier futur ajout de camp) ; (2) extraire le `CASE` dans une classe utilitaire neutre partagée par les deux (plus "propre" architecturalement mais touche `AdminUserController` de façon plus large que nécessaire pour un simple problème de visibilité) ; (3) retenue — passer `winCaseSql()` de `private` à `public static` dans `AdminUserController`, sans toucher sa signature ni son contenu, et l'appeler directement (`AdminUserController::winCaseSql()`) depuis `LeaderboardService`.
**Fix / Décision :** option (3) — changement d'une ligne dans `AdminUserController` (visibilité uniquement), aucun comportement existant modifié, les deux call sites internes (`$this->winCaseSql()`) restent valides sans changement. `LeaderboardService` (nouveau, `app/Services/Admin/`) importe et appelle la méthode statique pour ses 3 classements (top victoires, top parties jouées, top taux de victoire) — même logique de camp que `admin/users`, y compris le traitement `lovers`/`lover_player_id`.
**Leçon :** quand une méthode de calcul déjà écrite doit devenir réutilisable ailleurs et que son contenu ne change pas, préférer élargir sa visibilité (`private` → `public static`) plutôt que la dupliquer ou la déplacer — une extraction plus "propre" est un refactor à part, à ne pas mélanger avec une tâche qui demande explicitement la réutilisation sans réécriture.
**Statut :** 🔵 Choix assumé

## [CHOIX] Vérification victoire Amoureux dans les stats admin — pas de bug trouvé, tests de régression ajoutés (fix/admin-stats-lovers-winrate)
**Contexte :** tâche démarrée sur la prémisse que `winCaseSql()` (`app/Http/Controllers/Admin/AdminUserController.php`) ne couvrirait pas le cas `winner_team = 'lovers'`, par analogie avec les bugs "Victoire des Amoureux non déclenchée..." (partie SFICZ8) et "Victoire des Amoureux annulée à tort..." (partie RIQPAZ) documentés plus haut dans ce fichier, ainsi que l'entrée "Écran de fin de partie et historique affichaient 'Loups'/'Annulée'..." — trois cas où le camp `lovers` avait effectivement été oublié ailleurs dans le code. `feat/admin-stats` était déjà mergée sur `dev` au démarrage de cette tâche → branche `fix/admin-stats-lovers-winrate` créée à la place, comme demandé en cas de branche déjà mergée.
**Symptôme / Problème :** en relisant `winCaseSql()` et son usage dans `index()`/`show()` (`AdminUserController`) ainsi que sa réutilisation dans `LeaderboardService`, la branche `WHEN games.winner_team = 'lovers' AND game_players.lover_player_id IS NOT NULL THEN 1` était déjà présente — voir l'entrée "Statistiques admin — calcul des victoires par camp (feat/admin-stats)" ci-dessus, qui documentait déjà ce choix dès l'implémentation initiale (point 1 de "Cause / Alternatives"). Aucune vue Blade admin ne recalcule de victoire indépendamment (elles consomment uniquement les variables déjà calculées par le Controller). Donc pas de bug à corriger.
**Cause / Alternatives :** la confusion vient du fait que le camp `lovers` a effectivement été oublié à plusieurs endroits du code jeu au fil des étapes Cupidon (inspection Voyante, tables rôle→icône, écran de fin/historique, `WinConditionChecker` sur résolutions différées) — cf. entrées listées en Contexte — ce qui rend l'hypothèse "encore un endroit oublié" plausible a priori. Mais côté stats admin, la règle `lovers` avait été anticipée dès la conception initiale de `winCaseSql()` (choix explicite, pas un oubli). Alternative envisagée : ne rien faire de plus que constater l'absence de bug. Écartée : aucune couverture de test n'existait pour `AdminUserController::index()`/`show()` (contrairement à `LeaderboardService`/`AdminLeaderboardController` qui ont `tests/Feature/Admin/AdminLeaderboardTest.php` et `tests/Unit/Services/Admin/LeaderboardServiceTest.php`, eux-mêmes sans cas `lovers`) — un futur refactor de `winCaseSql()` pourrait casser silencieusement le cas `lovers` sans qu'aucun test ne le détecte.
**Fix / Décision :** aucune modification de `winCaseSql()`, `AdminUserController`, ni des vues (rien à corriger). Ajout de `tests/Feature/Admin/AdminUserControllerTest.php` (nouveau fichier, périmètre stats admin) : couvre le comptage d'une victoire `lovers` via `lover_player_id` dans la liste (`index`, colonne `wins_count`) et dans le donut de la fiche utilisateur (`show`, `winsCount`/`lossesCount`/`winRate`), plus un cas de défaite hors `lovers` pour non-régression. `php artisan test` : 323 tests passent (les 8 échecs `BroadcastException` sur `WinConditionCheckerTest`/`CupidonActionTest` sont préexistants, dus à l'absence de Reverb en local, indépendants de cette tâche — cf. entrée "Cupidon Étape 7" plus haut).
**Leçon :** avant de corriger un bug rapporté par analogie avec des bugs déjà connus sur le même thème (ici : "le camp `lovers` a été oublié X fois, il est probablement oublié ici aussi"), relire le code réel plutôt que de faire confiance à la généralisation — l'entrée DECISIONS.md documentant la décision initiale (`winCaseSql()`) aurait dû être consultée avant de démarrer l'implémentation. Quand une prémisse de tâche s'avère fausse, le travail utile qui reste est de combler le vrai trou (ici : absence de test de régression), pas de fabriquer un changement pour justifier la tâche.
**Statut :** 🔵 Choix assumé

## [CHOIX] Leaderboard admin — champ de date et périmètre des parties comptées (feat/admin-leaderboard)
**Contexte :** filtre de période ("Ce mois-ci" / "Mois précédent" / "Tout") sur `admin/leaderboard`, implémenté dans `LeaderboardService::periodRange()`/`baseQuery()`.
**Symptôme / Problème :** la consigne posait deux questions ouvertes sans trancher : (1) filtrer sur `game->created_at` ou `game->finished_at` si la colonne existe ; (2) le classement "Top parties jouées" ("tous résultats confondus") doit-il inclure les parties `finished` mais annulées (`winner_team = null`) ?
**Cause / Alternatives :**
1. Champ de date — `games.finished_at` existe bien (colonne nullable, casts `datetime`, voir `app/Models/Game.php` et la migration de création). `created_at` aurait mélangé des parties commencées un mois mais conclues le mois suivant dans la mauvaise période ; `finished_at` correspond exactement à la question posée par un classement mensuel ("qui a bien joué/gagné CE mois-ci").
2. Parties annulées dans "Top parties jouées" — alternative écartée : les inclure (lecture littérale de "tous résultats confondus"), ce qui aurait rendu les 3 classements incohérents entre eux (le dénominateur du taux de victoire, lui, doit nécessairement exclure les annulées pour que `wins/games` ait un sens — voir décision "Statistiques admin" ci-dessus) et un peu trompeur (une partie annulée n'a produit aucun résultat, gagné ou perdu). Retenue : les 3 classements partagent la même requête de base (`baseQuery()`), filtrée sur `games.status = 'finished' AND games.winner_team IS NOT NULL` — "tous résultats confondus" est interprété comme "victoire ou défaite", pas "y compris aucun résultat".
**Fix / Décision :** `LeaderboardService::baseQuery()` : `whereBetween('games.finished_at', [...])` pour les périodes bornées (`current_month`/`previous_month`, via `Carbon::startOfMonth()/endOfMonth()`, `subMonthNoOverflow()` pour le mois précédent), aucune borne pour `all`. `whereNotNull('games.winner_team')` appliqué uniformément aux 3 classements.
**Leçon :** quand un filtre de période s'applique à plusieurs classements dérivés d'une même notion ("partie jouée"), la cohérence entre classements (mêmes lignes de base, même dénominateur) prime sur une lecture littérale isolée de chaque libellé UI — sinon les 3 tableaux d'une même page racontent des histoires légèrement différentes sans que ce soit visible à l'écran.
**Statut :** 🔵 Choix assumé
